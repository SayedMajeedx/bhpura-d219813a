...
| **[`behaviour-tests.md`](./behaviour-tests.md)**                                           | Phase 6: how to turn source-text tests into behaviour tests, and the classification of every `readFileSync` test (convert / migration contract / guard). |
| **[`agent-handoff.md`](./agent-handoff.md)**                                               | Where the roadmap work stands, the owner's rules, the next steps and the working method; the first file to read when continuing it.                      |
| **[`bug-backlog.md`](./bug-backlog.md)**                                                   | Real bugs found during refactors, recorded instead of fixed; each entry has location, effect and suggested fix.                                          |
