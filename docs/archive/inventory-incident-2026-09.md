...
Because the second migration dropped only its _new_ trigger name (`trg_orders_restore_stock_on_delete`) and omitted dropping the old name (`orders_restore_stock_on_delete_trg`), both triggers remained registered and active in Postgres.
