-- Migration: 20260918170000_fix_order_contact_snapshot_checkout_email.sql
-- Description: Fixes customer email desynchronization during storefront checkout.
-- Ensures checkout-provided contact details (name, email, phone) are authoritative
-- and permanently preserved on orders, without being silently overwritten by stale CRM triggers.

-- 1. Redefine preserve_order_contact_snapshot so it preserves snapshots against NULL/empty overwrites,
--    while allowing explicit new contact details provided by checkout or administration.
...
AS $$
BEGIN
  NEW.customer_name_snapshot := COALESCE(NULLIF(trim(NEW.customer_name_snapshot), ''), OLD.customer_name_snapshot);
  NEW.customer_email_snapshot := COALESCE(NULLIF(trim(NEW.customer_email_snapshot), ''), OLD.customer_email_snapshot);
  NEW.customer_phone_snapshot := COALESCE(NULLIF(trim(NEW.customer_phone_snapshot), ''), OLD.customer_phone_snapshot);
  RETURN NEW;
END;
$$;

-- 2. Update place_storefront_order_core to atomically link customer and set contact snapshots
...
BEGIN
  ...
  IF v_uid IS NOT NULL THEN
    ...
    LIMIT 1;
  ELSE
    -- Storefront checkouts match any pre-existing CRM row for this brand by email first, then phone
  ...
  IF v_matched_customer_id IS NOT NULL
    ...
    -- Update customer_id and the authoritative order contact snapshots in the same statement
    -- so triggers never backfill stale snapshots from the previous order/CRM row.
    UPDATE public.orders
    SET
      customer_id = v_matched_customer_id,
  ...
  EXCEPTION WHEN unique_violation THEN
    -- Swallow email/phone unique constraint conflicts gracefully so order placement succeeds 100%
    NULL;
  END;

  -- Ensure authoritative contact snapshots for this specific purchase are always preserved
  UPDATE public.orders
  SET
    customer_name_snapshot = COALESCE(v_name, customer_name_snapshot),
    customer_email_snapshot = COALESCE(v_email, customer_email_snapshot),
...
-- 3. Data Repair: Fix customer_email_snapshot on order 1004 so records match checkout input
UPDATE public.orders
SET customer_email_snapshot = 'sayed@tcig.co'
WHERE id = '99edf600-141b-4d97-8549-f575693fe077'
  AND invoice_number = 1004;
