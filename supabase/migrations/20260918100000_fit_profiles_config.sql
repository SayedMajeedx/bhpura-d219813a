-- Phase 4: Configurable Fit Profiles
-- Adds fit_profiles column to business_settings and exposes it via brand_public_settings

-- 1) Add fit_profiles column to business_settings
ALTER TABLE public.business_settings
  ADD COLUMN IF NOT EXISTS fit_profiles jsonb NULL;

ALTER TABLE public.business_settings
  DROP CONSTRAINT IF EXISTS business_settings_fit_profiles_check;
ALTER TABLE public.business_settings
  ADD CONSTRAINT business_settings_fit_profiles_check
  CHECK (fit_profiles IS NULL OR jsonb_typeof(fit_profiles) = 'array');

-- 2) Recreate brand_public_settings view (append-only)
