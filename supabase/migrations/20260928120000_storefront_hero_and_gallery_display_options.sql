-- Storefront Hero Presentation & Gallery Display Options
-- Adds configurable layout, height, mobile video ratio, arrows, and PDP gallery aspect ratio.

ALTER TABLE public.business_settings
  ADD COLUMN IF NOT EXISTS hero_layout text NOT NULL DEFAULT 'full_bleed'
    CONSTRAINT hero_layout_check CHECK (hero_layout IN ('full_bleed', 'contained')),
  ADD COLUMN IF NOT EXISTS hero_height_desktop text NOT NULL DEFAULT 'standard'
    CONSTRAINT hero_height_desktop_check CHECK (hero_height_desktop IN ('compact', 'standard', 'cinematic')),
  ADD COLUMN IF NOT EXISTS hero_aspect_mobile text NOT NULL DEFAULT 'portrait_4_5'
    CONSTRAINT hero_aspect_mobile_check CHECK (hero_aspect_mobile IN ('portrait_4_5', 'story_9_16', 'square_1_1', 'landscape_4_3')),
  ADD COLUMN IF NOT EXISTS hero_show_arrows boolean NOT NULL DEFAULT true,
  ADD COLUMN IF NOT EXISTS pdp_gallery_aspect_ratio text NOT NULL DEFAULT '3:4'
    CONSTRAINT pdp_gallery_aspect_ratio_check CHECK (pdp_gallery_aspect_ratio IN ('3:4', '1:1', '4:5'));

COMMENT ON COLUMN public.business_settings.hero_layout IS 'Hero presentation mode: full_bleed (edge-to-edge luxury) or contained (boxed card with rounded corners)';
COMMENT ON COLUMN public.business_settings.hero_height_desktop IS 'Desktop hero height clamping: compact (400px), standard (500px), or cinematic (620px)';
COMMENT ON COLUMN public.business_settings.hero_aspect_mobile IS 'Mobile hero aspect ratio: portrait_4_5 (optimal for reels/videos), story_9_16, square_1_1, or landscape_4_3';
COMMENT ON COLUMN public.business_settings.hero_show_arrows IS 'Whether to show luxury slide chevron arrows on desktop/tablet';
COMMENT ON COLUMN public.business_settings.pdp_gallery_aspect_ratio IS 'Product detail page gallery aspect ratio: 3:4 (portrait), 1:1 (square), 4:5 (editorial)';

-- Recreate brand_public_settings view to expose new options
DROP VIEW IF EXISTS public.brand_public_settings CASCADE;

CREATE VIEW public.brand_public_settings
WITH (security_invoker = false) AS
SELECT
  bs.brand_id,
  bs.business_name,
  bs.logo_url,
  bs.currency,
  bs.primary_color,
  bs.text_color,
  bs.background_color,
  bs.font_family,
  bs.font_url,
  bs.cod_enabled,
  bs.card_enabled,
  bs.benefit_enabled,
  bs.benefit_qr_url,
  bs.footer_note,
  bs.delivery_fee,
  bs.pickup_enabled,
  bs.delivery_enabled,
  bs.logo_size,
  bs.logo_align,
  bs.header_bg,
  bs.header_fg,
  bs.footer_bg,
  bs.footer_fg,
  bs.heading_color,
  bs.link_color,
  bs.btn_primary_bg,
  bs.btn_primary_fg,
  bs.btn_secondary_bg,
  bs.btn_secondary_fg,
  bs.btn_checkout_bg,
  bs.btn_checkout_fg,
  bs.pages,
  bs.whatsapp_enabled,
  bs.whatsapp_number,
  bs.socials,
  bs.favicon_url,
  bs.show_header_name,
  bs.show_hero_title,
  bs.show_hero_about,
  bs.show_footer_name,
  bs.storefront_font_en,
  bs.storefront_font_ar,
  bs.hero_title_size,
  bs.hero_title_color,
  bs.hero_title_align,
  bs.storefront_font_en_url,
  bs.storefront_font_ar_url,
  bs.hero_title_en,
  bs.hero_title_ar,
  bs.storefront_accent_color,
  bs.storefront_background_color,
  bs.storefront_text_color,
  bs.digital_delivery_enabled,
  bs.menu_bg,
  bs.menu_fg,
  bs.menu_title_en,
  bs.menu_title_ar,
  bs.menu_show_home,
  bs.menu_show_account,
  bs.menu_show_orders,
  bs.menu_show_pages,
  bs.home_promo_cards,
  bs.show_new_arrivals,
  bs.show_best_sellers,
  bs.new_arrivals_title_en,
  bs.new_arrivals_title_ar,
  bs.best_sellers_title_en,
  bs.best_sellers_title_ar,
  bs.announcement_enabled,
  bs.announcement_text_en,
  bs.announcement_text_ar,
  bs.announcement_bg,
  bs.announcement_fg,
  bs.announcement_bold,
  bs.announcement_italic,
  bs.announcement_dismissible,
  bs.announcement_scope,
  bs.announcement_audience,
  bs.global_sale_badges_enabled,
  bs.cart_drawer_checkout_bg,
  bs.cart_drawer_checkout_fg,
  bs.vat_inclusive,
  bs.shipping_zones,
  bs.storefront_loader_text_en,
  bs.storefront_loader_text_ar,
  bs.storefront_radius,
  bs.header_glass,
  bs.badge_accent,
  bs.secondary_banner_parallax_enabled,
  bs.secondary_banner_parallax_mobile_enabled,
  bs.secondary_banner_parallax_breakpoint,
  bs.trending_banner_background_url,
  bs.category_banner_background_url,
  bs.homepage_editorial_sections,
  bs.storefront_typography,
  bs.product_title_color,
  bs.price_color,
  bs.footer_logo_size,
  bs.trust_badges,
  bs.storefront_mode,
  bs.catalog_show_prices,
  bs.catalog_inquiry_message_en,
  bs.catalog_inquiry_message_ar,
  bs.store_vertical,
  bs.store_modules,
  bs.fit_profiles,
  bs.delivery_estimate_enabled,
  bs.delivery_estimate_ar,
  bs.delivery_estimate_en,
  bs.brand_palette,
  bs.invoice_inherit_brand_color,
  bs.invoice_inherit_brand_font,
  -- Storefront 2.0 columns
  bs.storefront_design_version,
  bs.trust_bar_enabled,
  bs.trust_bar_position,
  bs.hero_overlay_strength,
  bs.hero_title_color_v2,
  bs.product_card_hover_image,
  bs.product_card_color_dots,
  bs.product_card_quick_add,
  bs.new_badge_days,
  bs.footer_layout,
  bs.footer_show_payment_methods,
  bs.newsletter_enabled,
  bs.newsletter_title_ar,
  bs.newsletter_title_en,
  bs.brand_story_enabled,
  bs.brand_story_image_url,
  bs.category_filters_enabled,
  bs.pdp_layout,
  bs.pdp_image_zoom,
  bs.social_proof_enabled,
  bs.recently_viewed_enabled,
  bs.motion_enabled,
  bs.quick_view_enabled,
  bs.back_in_stock_enabled,
  bs.fabric_care_ar,
  bs.fabric_care_en,
  bs.shipping_returns_ar,
  bs.shipping_returns_en,
  bs.business_hours_ar,
  bs.business_hours_en,
  bs.bundle_discount_percent,
  -- Hero presentation & PDP gallery options
  bs.hero_layout,
  bs.hero_height_desktop,
  bs.hero_aspect_mobile,
  bs.hero_show_arrows,
  bs.pdp_gallery_aspect_ratio
FROM public.business_settings bs
JOIN public.brands b ON b.id = bs.brand_id
WHERE b.is_active = true;

GRANT SELECT ON public.brand_public_settings TO anon, authenticated, service_role;

NOTIFY pgrst, 'reload schema';
