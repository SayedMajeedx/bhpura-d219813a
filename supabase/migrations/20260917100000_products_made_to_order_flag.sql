-- Migration: 20260917100000_products_made_to_order_flag.sql
-- Store Vertical & Modules Phase 3: Explicit is_made_to_order flag on products.
-- Separates made-to-order logic from custom_fields so that physical inventory products with
-- custom fields (e.g. gift messages, custom engravings) have stock checked and depleted,
-- while products flagged as is_made_to_order bypass stock deductions and route to location='custom'.

-- 1. Add is_made_to_order column
ALTER TABLE public.products
  ADD COLUMN IF NOT EXISTS is_made_to_order boolean NOT NULL DEFAULT false;

-- 2. Backfill: Existing products with custom fields are preserved as made-to-order (0 behavior change)
...
-- 3. Update get_storefront_page_data to return is_made_to_order for products
...
-- 4. Update place_storefront_order_internal_20260710 to read v_product.is_made_to_order
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Single source of truth: explicitly check is_made_to_order flag on the product
