...
BEGIN
  ...
  -- Step 1: Query only within public.customers (isolating completely from admin/staff directories in public.profiles)
  -- Match by active user session id
  ...
  -- Match by phone of registered account
  IF v_matched_customer_id IS NULL AND v_phone IS NOT NULL THEN
    SELECT id INTO v_matched_customer_id FROM public.customers
      WHERE brand_id = v_brand_id AND auth_user_id IS NOT NULL AND phone = v_phone LIMIT 1;
  END IF;
  ...
  -- Match by any guest phone
  IF v_matched_customer_id IS NULL AND v_phone IS NOT NULL THEN
    SELECT id INTO v_matched_customer_id FROM public.customers
      WHERE brand_id = v_brand_id AND phone = v_phone LIMIT 1;
  END IF;
  ...
  -- Combine guest orders that have the same email or mobile with a registered user in the same profile
...
-- Allow executing the function by public users
