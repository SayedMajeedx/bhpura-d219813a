...
async function googleAccessToken() {
  ...
  const unsigned = `${b64url({ alg: "RS256", typ: "JWT" })}.${b64url({ iss: account.client_email, scope: "https://www.googleapis.com/auth/firebase.messaging", aud: "https://oauth2.googleapis.com/token", iat: now, exp: now + 3600 })}`;
  ...
  if (!auth.ok) throw new Error(result.error_description || "Google OAuth failed");
  cachedGoogleToken = {
...
Deno.serve(async (req) => {
  ...
  for (const event of events ?? []) {
    ...
    let query = supabase
      ...
      .select("id,expo_push_token,token_provider")
      .eq("brand_id", event.brand_id)
      .eq("enabled", true);
    query = event.customer_id
      ? query.eq("customer_id", event.customer_id)
      : query.eq("marketing_enabled", true);
    query =
      event.event_type === "order_update"
        ? query.eq("order_updates_enabled", true)
        : query.eq("marketing_enabled", true);
    ...
    if (deviceError) {
      await supabase
        ...
        .update({
          status: "failed",
          last_error: deviceError.message,
    ...
    if (!devices?.length) {
      await supabase
        .from("customer_push_events")
        .update({ status: "processed", processed_at: now, recipient_count: 0 })
        .eq("id", event.id);
      continue;
    }
    ...
    const messages = expoDevices.map((device) => ({
      ...
      data: {
        ...
        url: event.target_url
          ? new URL(event.target_url, "https://pura.boutq.store").toString()
          : "https://pura.boutq.store",
      },
    }));
    try {
      const tickets: any[] = [];
      ...
      if (directDevices.length) {
        const { data: app } = await supabase
          ...
          .select("firebase_project_id")
          .eq("brand_id", event.brand_id)
          .maybeSingle();
        if (!app?.firebase_project_id)
          throw new Error("Firebase project is not provisioned for this brand");
        const accessToken = await googleAccessToken();
        for (const device of directDevices) {
          const fcm = await fetch(
            `https://fcm.googleapis.com/v1/projects/${app.firebase_project_id}/messages:send`,
            {
              method: "POST",
              headers: {
              ...
              body: JSON.stringify({
                message: {
                  ...
                  android: {
                    priority: "HIGH",
                    notification: {
                      channel_id: event.event_type === "marketing" ? "pura-offers" : "pura-orders",
                      sound: "default",
                    },
                  },
                },
          ...
          const result = await fcm.json();
          tickets.push(
            fcm.ok
              ? { status: "ok", id: result.name }
              : {
                  status: "error",
                  message: result.error?.message,
                  details: {
                    error: fcm.status === 404 ? "DeviceNotRegistered" : result.error?.status,
                  },
                },
          );
        }
      ...
      for (let index = 0; index < orderedDevices.length; index += 1) {
        ...
        if (disabled)
          await supabase
            ...
            .eq("id", device.id);
        await supabase.from("customer_push_delivery_log").upsert(
          {
            event_id: event.id,
            device_id: device.id,
            status,
        ...
        if (status === "accepted") eventAccepted += 1;
        else eventFailed += 1;
      }
      await supabase
        .from("customer_push_events")
        .update({
          ...
          accepted_count: eventAccepted,
          failed_count: eventFailed,
        })
        .eq("id", event.id);
      ...
      await supabase
        ...
        .update({
          ...
          available_at: new Date(
            Date.now() + 60000 * Math.min(30, 2 ** event.attempts),
          ).toISOString(),
        })
        .eq("id", event.id);
    }
