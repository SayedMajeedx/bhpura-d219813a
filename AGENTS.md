...
- **No Network in Unit Tests**: [`tests/setup.ts`](./tests/setup.ts) rejects every `fetch` and WebSocket, because the Supabase client points at production. Mock the data layer; `vi.mock` needs the relative path (`../src/lib/data/x`) as well as the `@/` alias, or the real module runs and its request is refused.
