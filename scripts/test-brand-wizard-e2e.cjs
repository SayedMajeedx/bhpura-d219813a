...
async function main() {
  ...
  // 1. Open Brand Wizard
  const wizardBtn = page.locator('button:has-text("معالج إطلاق متجر"), button:has-text("Launch Brand Wizard")').first();
  ...
  const launchBtn = dialog.locator('button:has-text("إطلاق البراند والتهيئة الفورية"), button:has-text("Launch & Provision Store")');
  ...
  const finishBtn = dialog.locator('button:has-text("إغلاق وإنهاء"), button:has-text("Finish & Close")');
  ...
  try { fs.unlinkSync(testLogoPath); } catch {}
  ...
  if (brandErr || !brandRow) {
    throw new Error(`Brand with slug "${testSlug}" was not found in DB: ` + JSON.stringify(brandErr));
  }

  const brandId = brandRow.id;
  console.log(`Created Brand ID: ${brandId}, Slug: ${brandRow.slug}, Primary Color: ${brandRow.primary_color}`);
  ...
  if (settingsErr || !settingsRow) {
    throw new Error(`business_settings for brand "${brandId}" not found: ` + JSON.stringify(settingsErr));
  }

  console.log('Verifying business_settings for abayas:');
  console.log(` - store_vertical: ${settingsRow.store_vertical} (expected 'abayas')`);
  console.log(` - storefront_design_version: ${settingsRow.storefront_design_version} (expected 2)`);
  ...
  if (settingsRow.storefront_design_version !== 2) {
    throw new Error(`Expected storefront_design_version = 2, got: ${settingsRow.storefront_design_version}`);
  }
  if (!settingsRow.trust_badges || settingsRow.trust_badges.length < 3) {
    throw new Error(`Expected at least 3 trust badges for abayas, got: ${settingsRow.trust_badges?.length}`);
  }
  ...
  console.log(`Cleaned up test brand "${testSlug}" (${brandId}) and owner "${testOwnerEmail}" completely.`);
