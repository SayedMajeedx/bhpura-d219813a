...
async function verifyStagingVersion() {
  ...
  const { data: settings } = await sb
    ...
    .select("storefront_design_version")
  ...
  if (settings?.storefront_design_version !== 2) {
    ...
    await sb
      .from("business_settings")
      .update({ storefront_design_version: 2 })
      .eq("brand_id", brand.id);
...
async function run() {
  ...
  console.log("\n--- Step 2: Testing Video Fallback on HeroV2 ---");
  {
    ...
    console.log(
      `Hero Box when video blocked: height = ${heroBox?.height}px, width = ${heroBox?.width}px`,
    );
    ...
    await page.screenshot({
      path: path.join(OUTPUT_DIR, "hero-video-fallback-1280px.png"),
      fullPage: false,
    });
  ...
  console.log("\n--- Step 3: Testing Keyboard Navigation & Modals ---");
  {
    ...
    if (await productCard.isVisible()) {
      ...
      const quickViewBtn = page
        .locator(
          'button[aria-label*="سريع"], button[aria-label*="Quick"], button:has-text("معاينة سريعة")',
        )
        .first();
  ...
  for (const bp of BREAKPOINTS) {
    for (const loc of LOCALES) {
      ...
      const trustBar = page
        .locator("[data-trust-bar], text=توصيل, text=Delivery, text=دفع, text=Payment")
        .first();
      ...
      const accordion = page
        .locator('[data-accordion], [role="region"], button[aria-expanded]')
        .first();
