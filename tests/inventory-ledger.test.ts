...
describe("Inventory Ledger Architecture & Invariants (Block 1)", () => {
  ...
  it("a: apply_inventory_movement inserts immutable ledger row and updates product_variants atomically", () => {
    ...
    expect(functionsMigration).toContain("CREATE OR REPLACE FUNCTION public.apply_inventory_movement");
  ...
  it("b: Idempotency key replay returns existing movement without modifying stock", () => {
    ...
    expect(schemaMigration).toContain("UNIQUE INDEX IF NOT EXISTS uq_inventory_movements_brand_idempotency");
    expect(functionsMigration).toContain("WHERE brand_id = p_brand_id AND idempotency_key = p_idempotency_key");
  ...
  it("d: Concurrent mutations serialize via pg_advisory_xact_lock on the variant", () => {
    expect(functionsMigration).toContain("PERFORM pg_advisory_xact_lock(hashtext(p_variant_id::text));");
    expect(functionsMigration).toContain("SELECT * INTO v_variant\n  FROM public.product_variants\n  WHERE id = p_variant_id\n  FOR UPDATE;");
  });
  ...
  it("e: Order state transition from pending to cancelled releases allocations exactly once", () => {
    ...
    expect(orderMachineMigration).toContain("'cancelled', 'canceled', 'refunded', 'voided', 'failed'");
    ...
    expect(orderMachineMigration).toContain("DELETE FROM public.order_inventory_allocations WHERE order_id = p_order_id;");
  });
  ...
  it("f: Deleting an already-cancelled order does not release stock a second time", () => {
    ...
    expect(orderMachineMigration).toContain("FROM public.order_inventory_allocations\n    WHERE order_id = OLD.id");
    // When cancelled, order_inventory_allocations is already deleted. The delete trigger finds 0 rows, preventing double restore!
    expect(orderMachineMigration).toContain("DELETE FROM public.order_inventory_allocations WHERE order_id = OLD.id;");
  });
  ...
  it("g: Deleting an active pending order releases stock exactly once via allocations table", () => {
    expect(orderMachineMigration).toContain("DROP TRIGGER IF EXISTS trg_orders_restore_stock_on_delete");
    expect(orderMachineMigration).toContain("DROP TRIGGER IF EXISTS orders_restore_stock_on_delete_trg");
    expect(orderMachineMigration).toContain("CREATE OR REPLACE FUNCTION public.trg_orders_inventory_delete_proc()");
  ...
  it("h: replace_order_items and transition compute diffs without orphans and suppress trigger loops", () => {
    expect(orderMachineMigration).toContain("CREATE OR REPLACE FUNCTION public.replace_order_items");
    expect(orderMachineMigration).toContain("PERFORM set_config('inventory.suppress_items_trigger', '1', true);");
    expect(orderMachineMigration).toContain("PERFORM set_config('inventory.suppress_items_trigger', '0', true);");
    expect(orderMachineMigration).toContain("PERFORM public.order_inventory_transition(p_order_id);");
  ...
  it("i: Card order terminal payment failure changes status and delegates stock to state machine", () => {
    expect(orderMachineMigration).toContain("CREATE TRIGGER trg_release_card_stock_on_terminal_payment");
    ...
    expect(orderMachineMigration).not.toContain("UPDATE public.product_variants\n        SET stock_incubator = stock_incubator +");
  ...
  it("j: Incubator transfer RPCs use apply_inventory_movement for transfers, sales, and reversals", () => {
    expect(orderMachineMigration).toContain("CREATE OR REPLACE FUNCTION public.transfer_stock_to_incubator");
    expect(orderMachineMigration).toContain("p_location => 'main',\n      p_delta => -v_needed_from_main");
    expect(orderMachineMigration).toContain("p_location => 'incubator',\n      p_delta => v_needed_from_main");
    expect(orderMachineMigration).toContain("CREATE OR REPLACE FUNCTION public.return_stock_from_incubator");
    expect(orderMachineMigration).toContain("CREATE OR REPLACE FUNCTION public.record_incubator_sale");
    expect(orderMachineMigration).toContain("CREATE OR REPLACE FUNCTION public.reverse_incubator_sale");
  });
  ...
  it("k: Direct UPDATE on product_variants stock columns is revoked and rejected by guard trigger", () => {
    ...
    expect(functionsMigration).toContain("BEFORE UPDATE OF stock_main, stock_incubator ON public.product_variants");
    expect(functionsMigration).toContain("RAISE EXCEPTION 'DIRECT_STOCK_UPDATE_FORBIDDEN");
    expect(functionsMigration).toContain("REVOKE UPDATE (stock, stock_main, stock_incubator) ON public.product_variants FROM authenticated");
  });
  ...
  it("l: Nightly reconciliation job corrects cached column drift against ledger sums without mutating ledger", () => {
    expect(reconciliationMigration).toContain("CREATE OR REPLACE FUNCTION public.reconcile_inventory");
    expect(reconciliationMigration).toContain("SELECT COALESCE(SUM(delta), 0) INTO v_main_ledger_sum\n    FROM public.inventory_movements");
    expect(reconciliationMigration).toContain("set_config('inventory.apply_via_ledger', '1', true)");
    expect(reconciliationMigration).toContain("UPDATE public.product_variants\n      SET stock_main = v_main_ledger_sum");
