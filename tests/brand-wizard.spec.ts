...
test.describe("Brand Wizard Provisioning Suite", () => {
  ...
  test("opens Brand Wizard dialog and supports vertical selection and slug auto-generation", async ({
    page,
  }) => {
    ...
    const createBtn = page
      .locator(
        'button:has-text("إنشاء"), button:has-text("Create"), button:has-text("إضافة"), [data-brand-wizard-trigger]',
      )
      .first();
    if (await createBtn.isVisible()) {
      ...
      const nameInput = dialog
        .locator('input[placeholder*="Brand"], input[name*="name_en"], input[id*="name_en"]')
        .first();
      ...
      const verticalButtons = dialog.locator(
        'button:has-text("عبايات"), button:has-text("Abayas"), button:has-text("أزياء"), button:has-text("Fashion")',
      );
  ...
  test("validates required fields before allowing progression", async ({ page }) => {
    ...
    const createBtn = page
      .locator('button:has-text("إنشاء"), button:has-text("Create"), button:has-text("إضافة")')
      .first();
    if (await createBtn.isVisible()) {
      await createBtn.click();
      await page.waitForTimeout(600);
      ...
      if (await dialog.isVisible()) {
        ...
        const nextBtn = dialog
          .locator('button:has-text("التالي"), button:has-text("Next")')
          .first();
