...
describe("customers and inventory bulk selection", () => {
  ...
  it("brand-scopes both individual and bulk customer deletion", () => {
    // The delete itself lives in the customers data layer (its RPC arguments
    // are covered by tests/customers-data-layer.test.ts).
    const route = read("src/routes/_authenticated/admin.b.$slug.customers.tsx");
    expect(route).toContain("deleteCustomers(brandId, [id])");
    expect(route).toContain("deleteCustomers(brandId, ids)");
