...
test.describe("Product Card & Quick View Suite", () => {
  test("renders product cards with 3:4 aspect ratio and quick interaction triggers", async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 900 });
    await page.goto("/pura?preview=1&design=2", { waitUntil: "domcontentloaded" });
    await page.waitForTimeout(1500);

    // Look for product card containers
    const productCard = page.locator('article, [data-product-id], [class*="product-card"], a[href*="/product/"]').first();
  ...
  test("triggers Quick View or Quick Add on desktop hover", async ({ page }) => {
    ...
    if (await productCard.isVisible()) {
      ...
      const actionButton = page.locator('button[aria-label*="سريع"], button[aria-label*="Quick"], button:has-text("معاينة"), button:has-text("إضافة")').first();
