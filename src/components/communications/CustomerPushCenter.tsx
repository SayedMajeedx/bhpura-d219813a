...
export function CustomerPushCenter({ brandId, isAr }: { brandId: string; isAr: boolean }) {
  ...
  const hasRecipients =
    target === "all" ? installedCustomers.size > 0 : installedCustomers.has(target);
  ...
  const send = async () => {
    ...
    if (!title.trim() || !body.trim())
      return toast.error(isAr ? "أدخل عنوان ورسالة الإشعار" : "Enter a title and message");
    if (target !== "all" && !installedCustomers.has(target))
      return toast.error(
        isAr
          ? "هذا العميل لم يسجل جهازاً في التطبيق"
    ...
    if (error)
      return toast.error(
        isAr
          ? "تعذر جدولة الإشعار، يرجى المحاولة مرة أخرى."
          : "Failed to queue push notification. Please try again.",
      );
    setTitle("");
    setBody("");
    setTarget("all");
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-bold">
              {isAr ? "إرسال إشعار للعملاء" : "Send customer notification"}
            </h2>
            <p className="text-xs text-muted-foreground">
              {isAr
                ? `${installedCustomers.size} عميل لديهم التطبيق والإشعارات مفعلة`
                : `${installedCustomers.size} customers have active app notifications`}
            </p>
          </div>
        </div>
        <div className="mt-5 space-y-4">
            ...
            <Select value={target} onValueChange={setTarget}>
              ...
              <SelectContent>
                <SelectItem value="all">
                  {isAr ? "كل العملاء المشتركين في العروض" : "All marketing subscribers"}
                </SelectItem>
                {(customers.data ?? [])
                  .filter((c) => installedCustomers.has(c.id))
                  .map((c) => (
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder={isAr ? "وصل الجديد من Pura Line" : "New at Pura Line"}
            />
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              placeholder={isAr ? "اكتب رسالة قصيرة وواضحة..." : "Write a short, clear message..."}
            />
            <p className="text-end text-xs text-muted-foreground">{body.length}/500</p>
          </div>
          <Button
            onClick={send}
            disabled={sending || !hasRecipients || !title.trim() || !body.trim()}
          ...
          <p className="text-xs text-muted-foreground">
            {isAr
              ? "الإرسال الجماعي يصل فقط لمن فعّل «العروض والأخبار». تحديثات الطلبات تُرسل تلقائياً ولا تعتمد على هذا الخيار."
              : "Broadcasts reach only customers who enabled marketing. Order updates are automatic and independent."}
          </p>
        </div>
      </Card>
      <Card className="p-4 sm:p-6">
        <div className="flex items-center justify-between">
          <h2 className="font-bold">{isAr ? "آخر الإرسالات" : "Recent sends"}</h2>
          <Button
            variant="ghost"
            size="icon"
            onClick={() => history.refetch()}
            aria-label={isAr ? "تحديث السجل" : "Refresh history"}
            title={isAr ? "تحديث السجل" : "Refresh history"}
          >
        ...
        <div className="mt-4 space-y-2">
          {!history.data?.length ? (
            <p className="rounded-xl border border-dashed p-6 text-center text-sm text-muted-foreground">
              {isAr ? "لا توجد إشعارات مرسلة بعد" : "No notifications sent yet"}
            </p>
          ) : (
            history.data.map((row) => (
              <div key={row.id} className="rounded-xl border p-3">
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  ...
                  {isAr ? "وصل" : "accepted"} {row.accepted_count}/{row.recipient_count}
                </p>
              </div>
            ))
          )}
