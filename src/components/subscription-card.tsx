...
export function SubscriptionCard({ brand }: SubscriptionCardProps) {
  ...
  const [renewalIntent, setRenewalIntent] = useState<
    "renew" | "cancel" | "upgrade" | "downgrade" | null
  ...
  const handleUploadReceipt = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    const toastId = toast.loading(isAr ? "جاري رفع الإيصال وتأمينه..." : "Uploading receipt...");
  ...
  return (
    <div className="space-y-8">
      ...
      {renewalWindowOpen && (
        <Card className="overflow-hidden border-border shadow-md rounded-2xl">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-base font-bold">
              <CreditCard className="h-5 w-5 text-primary" />
              <span>
                {isAr
                  ? "دفع وتجديد الاشتراك عبر BenefitPay"
                  : "BenefitPay Manual Renewal & Transfer"}
              </span>
            </CardTitle>
          ...
          <CardContent className="space-y-4">
            ...
            {renewalIntent === "renew" && (
              <div className="grid gap-5 md:grid-cols-[220px_1fr] pt-2 border-t border-border">
