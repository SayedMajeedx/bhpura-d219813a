...
export function ImageCropperDialog({ open, imageSrc, aspect = 3 / 4, onCancel, onConfirm, busy, outputWidth, outputHeight, heroPreview = false, title, description }: Props) {
  const { lang } = useI18n();
  const isAr = lang === "ar";
  ...
  const adjustZoom = (delta: number) => setZoom((current) => Math.min(4, Math.max(1, Number((current + delta).toFixed(2)))));
  const reset = () => {
    setCrop({ x: 0, y: 0 });
    setZoom(1);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o && !isBusy) onCancel(); }}>
      <DialogContent className="max-h-[94dvh] max-w-3xl overflow-y-auto overscroll-contain p-0">
          ...
          <div className="flex items-start gap-3 border-b px-5 pb-4 pt-5 pe-12 sm:px-6 sm:pe-14">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {description || (isAr ? "اسحب الصورة لتحديد موضعها، ثم استخدم التكبير للحصول على النتيجة المناسبة." : "Drag to position the image, then fine-tune the framing with zoom.")}
              </p>
            </div>
          </div>
        </DialogHeader>
        <div className="space-y-4 px-5 sm:px-6">
          <div className="relative h-[min(46vh,410px)] min-h-60 w-full shrink-0 overflow-hidden rounded-2xl bg-neutral-950 shadow-inner">
            ...
            {outputWidth && outputHeight && (
              <span className="pointer-events-none absolute bottom-3 end-3 rounded-full bg-black/65 px-3 py-1 font-mono text-[11px] text-white backdrop-blur" dir="ltr">
                {outputWidth} × {outputHeight}
              </span>
            )}
          </div>
          ...
          <div className="rounded-xl border bg-muted/20 p-3 sm:p-4">
            <div className="mb-3 flex items-center justify-between gap-3">
              <label className="text-sm font-medium">{isAr ? "التكبير والموضع" : "Zoom & position"}</label>
              <Button type="button" variant="ghost" size="sm" className="min-h-11 gap-2" onClick={reset} disabled={isBusy}>
                <RotateCcw className="h-4 w-4" />
                {isAr ? "إعادة ضبط" : "Reset"}
              </Button>
            </div>
            <div className="flex items-center gap-3">
              <Button type="button" variant="outline" size="icon" className="h-11 w-11 shrink-0 rounded-full" onClick={() => adjustZoom(-0.15)} disabled={isBusy || zoom <= 1} aria-label={isAr ? "تصغير" : "Zoom out"}>
                <Minus className="h-4 w-4" />
              </Button>
              <Slider aria-label={isAr ? "مستوى التكبير" : "Zoom level"} min={1} max={4} step={0.05} value={[zoom]} onValueChange={(v) => setZoom(v[0] ?? 1)} />
              <Button type="button" variant="outline" size="icon" className="h-11 w-11 shrink-0 rounded-full" onClick={() => adjustZoom(0.15)} disabled={isBusy || zoom >= 4} aria-label={isAr ? "تكبير" : "Zoom in"}>
                <Plus className="h-4 w-4" />
              </Button>
              <span className="hidden w-14 text-end font-mono text-xs text-muted-foreground sm:block">{Math.round(zoom * 100)}%</span>
            </div>
          </div>

        {heroPreview && imageSrc && <details className="group rounded-xl border bg-background">
        ...
        <DialogFooter className="sticky -bottom-1 z-10 mt-1 border-t bg-background/95 px-5 py-4 backdrop-blur sm:px-6">
          <Button variant="outline" className="min-h-11" onClick={onCancel} disabled={isBusy}>{isAr ? "إلغاء" : "Cancel"}</Button>
          <Button className="min-h-11 min-w-36" onClick={handleConfirm} disabled={isBusy || !area}>
            ...
            {processing ? (isAr ? "جاري تجهيز الصورة…" : "Preparing image…") : (isAr ? "اعتماد الصورة" : "Use this crop")}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
