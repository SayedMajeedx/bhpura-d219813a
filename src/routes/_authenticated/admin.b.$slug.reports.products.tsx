      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-background p-4 rounded-lg border">
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <Select value={sortBy} onValueChange={setSortBy}>
            ...
            <SelectContent>
              <SelectItem value="units_sold_desc">{lang === "ar" ? "الأكثر مبيعاً (كمية)" : "Highest Units Sold"}</SelectItem>
              <SelectItem value="net_merch_desc">{lang === "ar" ? "الأعلى قيمة (صافي البضائع)" : "Highest Net Merchandise"}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        <div className="flex items-center space-x-2">
          <Switch id="historical-products" checked={includeHistorical} onCheckedChange={setIncludeHistorical} />
          <Label htmlFor="historical-products">{lang === "ar" ? "تضمين الأرشيف" : "Include archived"}</Label>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {lang === "ar" ? "تفاصيل المبيعات والمخزون لكل منتج" : "Sales and stock details per product variant"}
            </CardDescription>
          </CardHeader>
          <CardContent>
            {productsData.length > 0 ? (
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-right">{lang === "ar" ? "المخزون الحالي" : "Current Stock"}</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {productsData.map((p: any, idx: number) => (
                      <TableRow key={idx}>
                        <TableCell className="font-medium">
                          {p.product_name}
                          {p.is_missing_cost && (
                            <Badge variant="destructive" className="ml-2 mt-1 text-[10px]" title={lang === "ar" ? "بيانات التكلفة مفقودة لهذا المنتج" : "Missing cost data for this product"}>
                        ...
                        <TableCell className="text-right">
                          <div className="flex items-center justify-end gap-2">
                            {p.is_out_of_stock ? (
                              <Badge variant="destructive" className="flex gap-1"><PackageX className="w-3 h-3" /> 0</Badge>
                            ) : p.is_low_stock ? (
                              <Badge variant="secondary" className="text-amber-500 border-amber-500/20 bg-amber-500/10">{p.current_stock}</Badge>
                            ) : (
              ...
              <div className="text-center p-12 border rounded-lg bg-muted/20">
                <p className="text-muted-foreground">{lang === "ar" ? "لا توجد بيانات لهذه الفترة" : "No product sales data available for this period."}</p>
              </div>
            )}
          </CardContent>
        </Card>
