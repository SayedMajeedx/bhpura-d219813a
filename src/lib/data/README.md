...
| Returns: the dashboard's pending count, the order editor's linked returns, the shopper's returns, the return policy (editor, readiness checklist, return request); admin list/detail/dialogs still direct (bug backlog #22)                                                                              | `returns/`           | orders, dashboard, accounting and expenses screens; storefront routes and slices; settings cards; the policy editor                                                                    |
