...
// Overview metrics
export async function fetchReportingOverview(range: DateRange, tz: string, includeHistorical: boolean = false, brandSlug?: string) {
  ...
  if (error) {
    ...
    if (error.code === "PGRST202" || /rpc_reporting_overview.*(not find|does not exist)/i.test(error.message || "")) {
      const sales = await fetchReportingSales(range, "day", tz, includeHistorical, brandSlug) as any;
      ...
      return [{
        ...
        net_merch_sales: series.reduce((sum: number, row: any) => sum + Number(row.net_merch || 0), 0),
        paid_order_count: series.reduce((sum: number, row: any) => sum + Number(row.paid_order_count || row.order_count || 0), 0),
        discounts: series.reduce((sum: number, row: any) => sum + Number(row.discounts || 0), 0),
        shipping_collected: series.reduce((sum: number, row: any) => sum + Number(row.shipping_collected || 0), 0),
        vat_collected: series.reduce((sum: number, row: any) => sum + Number(row.vat_collected || 0), 0),
        expenses: 0,
        partial_amount: 0,
        refunded_total: 0,
...
// Sales metrics
export async function fetchReportingSales(range: DateRange, interval: ReportInterval, tz: string, includeHistorical: boolean = false, brandSlug?: string) {
...
export async function fetchReportingProducts(range: DateRange, tz: string, includeHistorical: boolean = false, limit: number = 50, offset: number = 0, sortBy: string = 'units_sold_desc', brandSlug?: string) {
...
// Customers metrics
export async function fetchReportingCustomers(range: DateRange, tz: string, includeHistorical: boolean = false, limit: number = 50, offset: number = 0, brandSlug?: string) {
...
// Export function
export async function exportReportData(reportType: "sales" | "products" | "customers", range: DateRange, tz: string, brandSlug?: string) {
