...
export function resolveRouteTitles(path: string, fallbackTitle?: string): { en: string; ar: string } {
  ...
  if (cleanPath.endsWith("/inventory")) return { en: "Inventory & Catalog", ar: "المنتجات والمخزون" };
  ...
  if (cleanPath.endsWith("/reports/customers")) return { en: "Customer Reports", ar: "تقارير العملاء" };
  if (cleanPath.endsWith("/reports/products")) return { en: "Product Reports", ar: "تقارير المنتجات" };
  ...
  if (cleanPath.endsWith("/discounts")) return { en: "Discounts & Promos", ar: "الخصومات والكوبونات" };
  if (cleanPath.endsWith("/campaigns")) return { en: "Marketing Campaigns", ar: "الحملات والتسويق" };
  ...
  if (cleanPath.endsWith("/integrations")) return { en: "Integrations & API", ar: "الربط والتكامل" };
  if (cleanPath.includes("/admin/super/requests")) return { en: "Platform Control", ar: "تحكم المنصة الخارقة" };
